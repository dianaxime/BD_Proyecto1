...
AND date_actual IS null AND first_day_of_week IS not null AND last_day_of_week is not NULL""",(anio,inicio, fin,))
    ...
    def generarCsv(self):
        ...
        try:
            ...
            anio=self.inputCantidad.text()
            ##escritura a .csv
            with open('ventasSemanalesReporteria.csv', mode='w', newline='') as cvs_file:
                csv_writer = csv.writer(cvs_file, delimiter=';', quotechar='"', quoting=csv.QUOTE_MINIMAL)
                csv_writer.writerow(['Semana', 'Inicio', 'Fin','Total','Ventas'])
                if inicio != '' and fin != '' and anio!='':
                    cur.execute( """SELECT week_of_year, first_day_of_week, last_day_of_week, sum, count
                    FROM ventasdatacube
                    WHERE year_actual = %s AND
                        quarter_actual IS NULL AND month_actual IS NULL AND genero IS NULL AND artista is null and 
                        week_of_year IS NOT null and week_of_year >= %s and week_of_year <= %s
                    AND date_actual IS null AND first_day_of_week IS not null AND last_day_of_week is not NULL""",(anio,inicio, fin,))#print (cur.fetchall())
                    for a,b,c,d,e in cur.fetchall():
                        csv_writer.writerow([str(a), str(b), str(c), str(d), str(e)])
            # Cerremos el cursor
            cur.close()
        except (Exception, psycopg2.DatabaseError) as error:
            print(error)
