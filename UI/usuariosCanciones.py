...
# Form implementation generated from reading ui file 'usuariosCanciones.ui'
#
...
import csv
class Ui_usuariosCanciones(object):
    def conectar(self):
...
"color: rgb(72, 72, 72);")
        ...
        nombreColumnas = ("Usuario","Cantidad de canciones registradas")
...
"color: rgb(72, 72, 72);")
    ...
    def retranslateUi(self, Form):
        ...
        Form.setWindowTitle(_translate("Form", "Usuarios con mas canciones registradas"))
        self.volverButton.setText(_translate("Form", "Volver"))
        self.reporteButton.setText(_translate("Form", "CSV"))
        self.titutloLabel.setText(_translate("Form", "<html><head/><body><p align=\"center\"><span style=\" font-size:16pt; font-weight:600;\">Usuarios con más canciones</span></p></body></html>"))
    ...
    def generarCsv(self):
        ...
        try:
            ...
            db_version = cur.fetchone()
            ##escritura a .csv
            with open('usuarioMasRegistradasConsult.csv', mode='w', newline='') as cvs_file:
                csv_writer = csv.writer(cvs_file, delimiter=';', quotechar='"', quoting=csv.QUOTE_MINIMAL)
                csv_writer.writerow(['Usuario', 'Cantidad registrada'])
                cur.execute( "SELECT customer.firstname, COUNT(creador_track.creadorid) FROM creador_track JOIN customer ON customer.customerid=creador_track.creadorid GROUP BY customer.customerid ORDER BY COUNT(creador_track.creadorid) DESC LIMIT 5" )
                #print (cur.fetchall())
                for a,b in cur.fetchall():
